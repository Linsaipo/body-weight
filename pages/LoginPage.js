...
import { GoogleAuthProvider, OAuthProvider, signInWithPopup, signInWithRedirect } from "https://www.gstatic.com/firebasejs/12.4.0/firebase-auth.js";

export default function LoginPage() {
  const wrap = document.createElement('div');
  wrap.className = 'max-w-md mx-auto py-10';
  wrap.innerHTML = `
    <div class="bg-white rounded-2xl shadow p-6">
      <h1 class="text-xl font-bold mb-4">登入 PharmaFit</h1>
      <div class="space-y-3">
  ...
  wrap.querySelector('#btnGoogle').addEventListener('click', async ()=>{
    try{ await signInWithPopup(auth, googleProvider); }catch{ try{ await signInWithRedirect(auth, googleProvider);}catch(e){ alert('Google 登入失敗：'+(e?.message||e)); } }
  });
  wrap.querySelector('#btnApple').addEventListener('click', async ()=>{
    try{ await signInWithPopup(auth, appleProvider); }catch{ try{ await signInWithRedirect(auth, appleProvider);}catch(e){ alert('Apple 登入失敗：'+(e?.message||e)); } }
  });
