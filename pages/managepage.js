...
import {
  ...
  orderBy, limit, doc, updateDoc, addDoc, serverTimestamp
...
export default {
  ...
  async mount(ctx){
    ...
    if (isCoachLike){
      ...
      membersBody.innerHTML = `
        <tr><td class="px-3 py-4 text-slate-500" colspan="4">僅教練 / Admin 可查看會員清單。</td></tr>
      `;
    }
    ...
    function openMemberDetail(memberUid, title){
      ...
      unsubDetail = onSnapshot(qy, snap=>{
        ...
        const points = rows
          ...
          .map(r=>({ x: r.date?.toDate?.()? r.date.toDate(): new Date(r.date), y: Number(r.weight) }));

        const data = {
          datasets: [{
            label:'體重 (kg)',
            data: points,
            borderColor:'#2563eb', borderWidth:2, tension:0.3, pointRadius:3, fill:false
          }]
        };
        const options = {
          ...
          plugins:{ tooltip:{ enabled:true, callbacks:{ label:c=>`體重: ${c.parsed.y} kg` }}, legend:{ display:true } },
    ...
    sendInviteBtn?.addEventListener('click', async ()=>{
      try{
        ...
        const isCoach = isCoachLike;
        const payload = {
          type: isCoach ? 'coach_to_member' : 'member_to_coach',
          fromUid: user.uid,
          fromEmail: meEmail,
          toEmail,
          memberUid: isCoach ? null : user.uid, // 教練端可為 null；會員端一定是本人
          status: 'pending',
    ...
    const renderInbox = ()=>{
      ...
      for(const [id, inv] of box){
        ...
        tr.querySelector('.accept')?.addEventListener('click', async ()=>{
          ...
          }catch(e){ alert('同意失敗：'+(e?.message||e)); }
        });
    ...
    const unsubTo   = onSnapshot(toQ,   snap=>{ for(const ch of snap.docChanges()){ if(ch.type==='removed') box.delete(ch.doc.id); else box.set(ch.doc.id,{id:ch.doc.id,...ch.doc.data()}); } renderInbox(); });
    const unsubFrom = onSnapshot(fromQ, snap=>{ for(const ch of snap.docChanges()){ if(ch.type==='removed') box.delete(ch.doc.id); else box.set(ch.doc.id,{id:ch.doc.id,...ch.doc.data()}); } renderInbox(); });
  }
};