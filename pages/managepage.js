...
function fmtDate(tsOrDate) {
  ...
  const dd = String(d.getDate() + 0).padStart(2, '0'); // 0 protection
...
export default {
  ...
  async mount(ctx) {
    ...
    let chart, unsubDetail = null, unsubMembers = null;

    /* ========== 我的會員清單（admin 全部 / coach 僅自己） ========== */
    async function mountMembers() {
      if (unsubMembers) { unsubMembers(); unsubMembers = null; }

      const renderRows = async (snap) => {
        membersBody.innerHTML = '';
        if (snap.empty) {
          membersBody.innerHTML = `<tr><td colspan="4" class="px-3 py-4 text-slate-500">目前沒有會員。</td></tr>`;
          return;
        }
        for (const d of snap.docs) {
          ...
          tr.querySelector('.view')?.addEventListener('click', () => {
            openMemberDetail(d.id, u.displayName || u.email || d.id);
          });
      ...
      if (profile?.role === 'admin') {
        // admin：看全部 users
        const qAllUsers = collection(db, 'users');
        unsubMembers = onSnapshot(qAllUsers, renderRows);
      } else if (profile?.role === 'coach') {
        // 教練：只看自己帶的
        const qMyMembers = query(collection(db, 'users'), where('coachEmail', '==', meEmail));
    ...
    /* ========== 發送邀請（全用 email） ========== */
    async function sendInvite(toEmailRaw) {
      ...
      const type = isCoach ? 'coach_to_member' : 'member_to_coach';

      // 教練→會員：memberUid 可為 null（規則允許）
      // 會員→教練：memberUid 必為本人 uid
      const payload = {
        type,
        fromUid: user.uid,
        fromEmail: meEmail,
        toEmail,
        memberUid: isCoach ? null : user.uid, // 只有會員→教練才一定帶本人 uid
        status: 'pending',
