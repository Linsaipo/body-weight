...
export default {
    ...
    return `
      <section class="max-w-6xl mx-auto py-6">
        <div class="bg-white rounded-2xl shadow p-6">
          ...
          <div id="guard" class="hidden text-slate-600">只有管理員可以使用此頁面。</div>
  ...
  async mount(ctx){
    ...
    if (!profile || profile.role!=='admin'){
      ...
      rows.innerHTML = `<tr><td class="px-3 py-4 text-slate-500" colspan="5">無權限</td></tr>`;
      return;
    }
    ...
    for(const d of ss.docs){
      ...
      tr.querySelector('.save').addEventListener('click', async ()=>{
        ...
        try{ await updateDoc(doc(db,'users', d.id), { role }); alert('已更新'); }
